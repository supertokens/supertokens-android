...
public class SuperTokensHttpsRequest {
    private static final Object refreshTokenLock = new Object();

    public static <T> T newRequest(Context context, URL url, SuperTokensHttpsCallback<T> callback) throws IllegalAccessException, IOException {
        ...
        try {
            while (true) {
                ...
                if ( responseCode == SuperTokens.sessionExpiryStatusCode ) {
                    ...
                    boolean retry = SuperTokensHttpsRequest.handleUnauthorised(context, preRequestIdRefreshToken);
                    if(!retry) {
                        return output;
                    }
                } else {
                    ...
                    SuperTokensHttpsRequest.saveIdRefreshTokenFromSetCookie(context, cookies, cookieManager);

                    // Store the anti-CSRF token from the response headers
                    SuperTokensHttpsRequest.saveAntiCSRFFromConnection(context, connection);
    ...
    private static boolean handleUnauthorised(Context context, String preRequestIdRefreshToken) throws IOException {
        ...
        synchronized (refreshTokenLock) {
            ...
            HttpsURLConnection refreshTokenConnection = (HttpsURLConnection) refreshTokenUrl.openConnection();
            ...
            SuperTokensHttpsRequest.saveAntiCSRFFromConnection(context, refreshTokenConnection);
    ...
    private static void saveAntiCSRFFromConnection(Context context, HttpsURLConnection connection) {
    ...
    public interface SuperTokensHttpsCallback<T> {
        T doBody(HttpsURLConnection con);
    }
}
